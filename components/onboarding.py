"""
Onboarding screen — mobile-first consumer welcome flow.
"""
...
def render_onboarding() -> None:
    ...
    st.markdown("""
    <div style="
        background: linear-gradient(135deg, #1B4F72 0%, #2E86AB 60%, #85C1E9 100%);
        border-radius: 20px;
    ...
    ">
        ...
        <p style="color:rgba(255,255,255,0.85); font-size:1rem; margin:0;">
            Join thousands of consumers generating real-world health evidence
        </p>
    </div>
    ...
    for col, icon, label in zip(
        ...
        with col:
            ...
            <div style="text-align:center; padding:0.8rem; background:white;
                ...
                <div style="font-size:0.8rem; color:#5D6D7E; font-weight:600; margin-top:0.3rem;">
                    {label}
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    <p style="text-align:center; color:#AEB6BF; font-size:0.78rem; margin-top:1.5rem;">
        Your data is used only to generate aggregated population insights.
